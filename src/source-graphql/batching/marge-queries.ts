  ...
  queries.forEach((query, index) => {
    ...
    prefixedQuery.query.definitions.forEach((def) => {
      if (isQueryDefinition(def)) {
        let _def$variableDefiniti

        mergedSelections.push(...def.selectionSet.selections)

        mergedVariableDefinitions.push(
          ...((_def$variableDefiniti = def.variableDefinitions) !== null && _def$variableDefiniti !== void 0
            ? _def$variableDefiniti
            : [])
        )
      }
...
export const resolveResult = (mergedQueryResult) => {
